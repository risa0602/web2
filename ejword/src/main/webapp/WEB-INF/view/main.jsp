...
<option value="startsWith"<%=ejw.getMode().equals("statsWith")?"selected":""%>>で始まる</option>
...
<option value="match"<%=ejw.getMode().equals("match")?"selected":""%>>で一致する</option>
</select>
	...
	<%}else{ %>
		...
		<%--件数が全件数に届かないときは次へのリンクを表示 --%>
			<%if(ejw.getPageNo()*ejw.getLimit() < ejw.getTotal()){ %>
				<li><a href="/ejword/main?searchWord=<%=ejw.getSearchWord() %>&mode=<%=ejw.getMode() %>&page=<%=ejw.getPageNo()+1 %>"><span aria-hidden="true">&rarr;</span>次へ</a></li>
			<%} %>
		</ul>
	<%} %>
