...
<link rel="skylesheet" href="/ejword/css/main.css">
</head>
<body>
<div class="container">
...
<option value="startsWith"${ ejw.mode eq "statsWith"?"selected":""}>で始まる</option>
...
<option value="match"${ejw.mode eq "match"?"selected":""}>で一致する</option>
</select>
<button type="submit" class="btn btn-primany">検索</button>
</form>
...
<c:if test="${not empty ejw.list }">
	<c:if test="${ejw.total le ejw.limit }">
		...
		${ejw.pageNo*ejw.limit gt ejw.total ? ejw.total:ejw.pageNo*ejw.limit }件を表示
		</p>
		<ul class="pager">
		<c:if test="${ejw.pageNo gt 1 }">
				<li><a href="/ejword/main?searchWord=${ejw.getsearchWord} &mode=${ejw.mode }&page=${ejw.pageNo()-1}" >←前へ</a></li>
				</c:if>
	...
	<table class="table table-borderd table-striped">
		<c:forEach ver="w" items=${ejw.list }">
			<tr><th>${w.gettitle}</th><td>${w.body}</td></tr>
			</c:forEach>
	</table>
<c:if test="${not empty ejw.pageir }">
